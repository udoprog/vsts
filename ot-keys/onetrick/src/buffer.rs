...
impl ResizableBuffer {
    ...
    pub fn set_working_block(&mut self, block_start: usize, block_end: usize) {
        unsafe {
            ...
            self.buffer.set_slices(block_length, |buffer_slices| {
                ...
                for (buffer_slice, channel_storage) in
                    buffer_slices.iter_mut().zip(self.buffer_storage.iter_mut())
                {
                    *buffer_slice = &mut *(&mut channel_storage.as_mut_slice()
    ...
    pub fn reset_working_block(&mut self) {
        unsafe {
            self.buffer
                .set_slices(self.real_samples(), |buffer_slices| {
                    // Swap out pointers to our internal buffer storage
                    for (buffer_slice, channel_storage) in
                        buffer_slices.iter_mut().zip(self.buffer_storage.iter_mut())
                    {
    ...
    */
    ...
    fn real_samples(&self) -> usize {
        if !self.buffer_storage.is_empty() {
            self.buffer_storage[0].len()
        } else {
            0
        }
    }
...
impl InternallyResampledBuffer {
    ...
    pub fn setup(
        ...
        let required_internal_samples =
            (sample_count as f32 * self.internal_sample_rate_ratio) as usize + 3;
        ...
        self.external_buffer_output
            .resize(output_channel_count, required_external_samples);
        //self.internal_buffer_input.resize(input_channel_count, required_internal_samples);
        self.internal_buffer_output
            .resize(output_channel_count, required_internal_samples);
    }
    pub fn process_samples<ProcessFn>(
        &mut self,
        sample_count: usize,
        _inputs: &[&[f32]],
        outputs: &mut [&mut [f32]],
        ...
        ProcessFn: FnMut(usize, &[&[f32]], &mut [&mut [f32]]), // Count, Input, Output
    {
        ...
        let requesting_internal_samples = (sample_count as f32 * self.internal_sample_rate_ratio
            - self.internal_playhead) as usize
            + 1;
        ...
        self.internal_buffer_output
            .set_working_block(start_internal_sample, end_internal_sample);
        process_samples(
            requesting_internal_samples,
            &[],
            self.internal_buffer_output.buffer.as_slice(),
        );
        self.internal_buffer_output.reset_working_block();

        let next_internal_playhead = self.internal_playhead
            + (requesting_external_samples as f32 * self.internal_sample_rate_ratio);

        // Copy from Internal buffer to External buffer
        self.external_buffer_output.buffer.fill_with_resampled(
            0,
            requesting_external_samples,
            &mut self.internal_buffer_output.buffer,
            self.internal_playhead,
            next_internal_playhead,
        );
        // Write to the actual output buffer slice
        self.external_buffer_output
            .buffer
            .write_to_slice_frames(outputs, requesting_external_samples);
        ...
        if truncate_internal_samples > 0 {
            self.internal_buffer_output
                .buffer
                .move_samples_to_front(truncate_internal_samples, truncate_internal_samples + 1);
...
pub trait BufferSlicer {
    ...
    fn slice_range_mut<const N: usize>(
        &mut self,
        start_index: usize,
        end_index: usize,
    ) -> [&mut [f32]; N];
    ...
    fn fill_with_resampled(
        ...
        for (from_channel, to_channel) in source.as_slice().iter().zip(self.as_slice().iter_mut()) {
            for i in 0..index_range {
                ...
                let value =
                    (from_channel[from_a] * (from_t - 1.0)) + (from_channel[from_b] * from_t);
...
impl<'buffer> BufferExtras for Buffer<'buffer> {
    ...
    fn add_to_buffer(&self, to_buffer: &mut Buffer) {
        for (self_channel, to_channel) in self
            .as_slice_actually_immutable()
            .iter()
            .zip(to_buffer.as_slice().iter_mut())
        {
    ...
    fn write_to_buffer_range(
        ...
        for (self_channel, to_channel) in self
            ...
            .zip(to_buffer.as_slice().iter_mut())
        {
            to_channel[to_min..to_max].copy_from_slice(&self_channel[from_min..from_max]);
        }
    }
    fn read_from_slice_range(
        &mut self,
        ...
        for (self_channel, from_channel) in self.as_slice().iter_mut().zip(from_slice.iter()) {
            self_channel[to_min..to_max].copy_from_slice(&from_channel[from_min..from_max]);
        }
    }
