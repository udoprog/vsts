...
use nih_plug_egui::egui::{
    Align2, Color32, CornerRadius, FontFamily, FontId, Painter, Pos2, Rect, Vec2,
};
...
impl DesignElementDrawer for Painter {
    ...
    fn fade_80s(&self, rect: Rect, count: u32, color: Color32) {
        ...
        for i in 0..count {
            ...
            self.rect_filled(line_rect, CornerRadius::ZERO, color);
        }
    }
    ...
    fn midi_keyboard(
        ...
        let white_keys_from_c = [
            true, false, true, false, true, true, false, true, false, true, false, true,
        ];
        let white_width_ratio: f32 = 12.0 / 7.0;
        let mut key_shapes: [Rect; 12] =
            [Rect::from_min_size(Pos2::new(0.0, 0.0), Vec2::new(0.0, 0.0)); 12];
        {
        ...
        let key_base_rect_template =
            Rect::from_min_size(rect.left_top(), Vec2::new(key_base_width, key_base_height));
        for pass in 0..2 {
            // White, then Black keys
            for (index, key) in (lowest_key..=highest_key).enumerate() {
                ...
                if is_white == (pass == 0) {
                    let mut key_rect = key_base_rect_template
                        .translate(Vec2::new(index as f32 * key_base_width, 0.0));
                    let key_shape = key_shapes[key % 12];
                    key_rect = key_rect.translate(Vec2::new(
                        key_base_width * key_shape.left(),
                        key_base_height * key_shape.top(),
                    ));
                    ...
                    self.rect_filled(key_rect, CornerRadius::ZERO, color);
                    if is_active {
                        let key_rect = if is_white {
                            key_rect
                        } else {
                            key_rect
                                .shrink2(Vec2::new(1.0, 0.5))
                                .translate(Vec2::new(0.0, -0.5))
                        };
                        self.rect_filled(key_rect, CornerRadius::ZERO, style.active_color);
                    }
                }
            }
        }
    ...
    fn vu_meter_mono(&self, rect: Rect, level: f32, style: &VuMeterStyle) {
        ...
        if style.bg_color.a() > 0 {
            self.rect_filled(rect, CornerRadius::same(style.padding), style.bg_color);
        }
        ...
        let tick_size = if is_vertical {
            Vec2::new(width, tick_length)
        } else {
            Vec2::new(tick_length, width)
        };
        ...
        let tick_spacing = if is_vertical {
            Vec2::new(0.0, tick_length + adjusted_spacing)
        } else {
            Vec2::new(tick_length + adjusted_spacing, 0.0)
        };
        ...
        for i in 0..style.dot_count {
            ...
            let index = if is_vertical {
                style.dot_count - i - 1
            } else {
                i
            };
            let ratio = index as f32 / style.dot_count as f32;
            let dot_color = if ratio > 0.89 {
                style.peak_color
            } else if ratio > 0.69 {
                style.warn_color
            } else {
            ...
            let color = if index < lit_dot_count {
                dot_color
            } else {
                style.unlit_color
            };
            //self.circle_filled(pos, radius, color);
            self.rect_filled(tick_rect, CornerRadius::same(1), color);
        }
    }
